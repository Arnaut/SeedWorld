...
int* find_fields(int* map, int cols, int rows, int* size)
{
	printf("in find fields\n");
  ...
  while((x<rows)&&(nb_cities>0))
  {
	  ...
	  if(pick_it>=0)//a city can be build
	      ...
	      else//city2 otherwise
		{
	      ...
	      y+=final_c*4;//space between cities
	    }
	  else
	    {
	      y++;
	...
	{
		printf("built\n");
	  x+=final_r*4;//space between cities
	}
      else
	{
	  x++;
